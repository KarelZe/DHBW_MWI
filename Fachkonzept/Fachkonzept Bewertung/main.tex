...
Anleihen sind kein Bestandteil der Anwendung \textit{TOPSIM}, insofern ist eine Bewertung durch die Anwendung \textit{Anika} notwendig. \gls{FRN} werden gemäß Definition \ref{def:floater} in der Anwendung \textit{Anika} efiniert.

\begin{definition}
	\label{def:floater}
	\gls{FRN} werden in Periode $P_0$ mit einer Laufzeit von 10 Perioden emittiert. Die Laufzeit eines Zinskupons beträgt eine Periode. \textit{payment date} und \textit{reset date} entsprechen dem Periodenultimo. Der Zinskupon setzt sich aus dem veränderlichen Kapitalmarktzins und einem emissionsindividuellen Spread zusammen.
\end{definition}
...
Kritisch zu beurteilen ist, dass dadurch keine . Für eine Bewertung von \gls{FRN}, die auch Ausfallrisiken berücksichtigt, wird auf \textcite[][]{brigo_interest_2006} verwiesen.
